(function() {
	d3.sequenceChart = function(options) {
		...
		this.ease = options.ease || d3.easeExpOut
		...
		this.draw = function() {
			...
			setTimeout(()=>{
				if(this.line) {
					...
					this.svg.selectAll(".seq-line").transition(lineTransition).style("stroke", this.lineColor);
				}

				this.svg.selectAll(".seq-block").on('mouseover', () => {
					d3.select(d3.event.target).style('fill', this.blockHoverColor)
				}, false);
				this.svg.selectAll(".seq-block").on('mouseout', () => {
